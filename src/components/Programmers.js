...
export default function Programmers() {
  ...
  const [focusedPioneer, setFocusedPioneer] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    const newObj = pioneers[focusedPioneer - 1]
      ...
      <div className='programmers'>
        {
          ...
          pioneers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFocusedPioneer(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
